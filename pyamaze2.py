...
class agent:
    ...
    def moveRightk(self,event):
        if self._parentMaze.maze_map[self.x,self.y]['E']==True:
            self.y=self.y+1
            self.pathk.append((self.x,self.y))
            if (self.x,self.y)==(self.goal):
                textLabel(self._parentMaze,'user cost',len(self.pathk)+1)
                textLabel(self._parentMaze,'Your Score',int((len(self._parentMaze.path))/(len(self.pathk))*100))
...
class maze:
    ...
    def enableArrowKey(self,a):
        ...
        '''
        self._win.bind('<Left>',a.moveLeftk)
        self._win.bind('<Right>',a.moveRightk)
        self._win.bind('<Up>',a.moveUpk)
        self._win.bind('<Down>',a.moveDownk)
